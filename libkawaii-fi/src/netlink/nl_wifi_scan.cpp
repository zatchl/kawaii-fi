...
namespace {
	...
	int update_ap(nl_msg *msg, void *aps)
	{
		...
		if (!attribute_index[NL80211_ATTR_BSS]) {
			return NL_SKIP;
		}

		// Create attribute index based on nested attribute
		if (nla_parse_nested(bss_attribute_index.begin(), NL80211_BSS_MAX,
		                     attribute_index[NL80211_ATTR_BSS], bss_policy.begin())) {
			return NL_SKIP;
		}

		if (!bss_attribute_index[NL80211_BSS_BSSID]) {
			return NL_SKIP;
		}

		auto access_points = static_cast<QVector<AccessPoint> *>(aps);
		if (!access_points) {
			return NL_SKIP;
		}
		...
		access_points->append(AccessPoint(bss_bssid, bss_attribute_index));
		return NL_SKIP;
	}
} // namespace
...
{
	KawaiiFi::NlWifiCommand<QVector<AccessPoint>> command(NL80211_CMD_GET_SCAN, NLM_F_DUMP,
	                                                      &update_ap, &aps);
