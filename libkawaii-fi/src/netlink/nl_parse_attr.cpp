...
QVector<IeVariant> parse_ies(nlattr *attr)
{
	QVector<IeVariant> ies;
	...
	while (element_start + element_length <= ies_bytes.size()) {
		...
		case WLAN_EID_COUNTRY:
			ies.append(CountryInfo(ie_data_bytes));
			break;
		case WLAN_EID_DS_PARAMS:
			ies.append(DsParameter(ie_data_bytes));
			break;
		case WLAN_EID_ERP_INFO:
			ies.append(ErpInfo(ie_data_bytes));
			break;
		case WLAN_EID_EXT_CAPABILITY:
			ies.append(ExtendedCapabilities(ie_data_bytes));
			break;
		case WLAN_EID_HT_CAPABILITY:
			ies.append(HtCapabilities(ie_data_bytes));
			break;
		case WLAN_EID_HT_OPERATION:
			ies.append(HtOperations(ie_data_bytes));
			break;
		case WLAN_EID_PWR_CONSTRAINT:
			ies.append(PowerConstraint(ie_data_bytes));
			break;
		case WLAN_EID_RRM_ENABLED_CAPABILITIES:
			ies.append(RmCapabilities(ie_data_bytes));
			break;
		case WLAN_EID_RSN:
			ies.append(RobustSecurityNetwork(ie_data_bytes));
			break;
		case WLAN_EID_SSID:
			ies.append(Ssid(ie_data_bytes));
			break;
		case WLAN_EID_SUPP_RATES:
		case WLAN_EID_EXT_SUPP_RATES:
			ies.append(SupportedRates(ie_data_bytes, ie_id));
			break;
		case WLAN_EID_TIM:
			ies.append(TrafficIndicationMap(ie_data_bytes));
			break;
		case WLAN_EID_VENDOR_SPECIFIC: {
			const VendorSpecific vendor_specific_ie(ie_data_bytes);
			if (vendor_specific_ie.oui() == WPA_OUI && vendor_specific_ie.type() == 1) {
				ies.append(Wpa(ie_data_bytes));
			} else {
		...
		case WLAN_EID_VHT_CAPABILITY:
			ies.append(VhtCapabilities(ie_data_bytes));
			break;
		case WLAN_EID_VHT_OPERATION:
			ies.append(VhtOperations(ie_data_bytes));
			break;
		default:
			ies.append(InformationElement(ie_data_bytes, ie_id));
		}
