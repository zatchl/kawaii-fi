...
namespace KawaiiFi::Ies {
	...
	QString CountryInfo::country_code() const
	{
		...
		return QString("%0%1")
		        .arg(bytes()[first_char_byte_index])
		        .arg(bytes()[second_char_byte_index]);
	}

	CountryInfo::Environment CountryInfo::environment() const
	{
	...
	QVector<ChannelPowerConstraint> CountryInfo::channel_constraints() const
	{
		...
		while (i + 2 < bytes().size()) {
			...
			channel_constraints.append({first_channel_number, last_channel_number,
			                            number_of_channels, max_transmit_power_dbm});
		}
