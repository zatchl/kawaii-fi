...
namespace KawaiiFi::Ies {
	    ...
	    : QStandardItem("HT Capabilities")
	{
		appendRow({new QStandardItem("LDPC Coding"),
		           new QStandardItem(ht_cap.ldpc_coding_capability() ? "Supported"
		                                                             : "Not Supported")});
		...
		case HtSupportedChannelWidth::TwentyOrFortyMhz:
			appendRow({new QStandardItem("Supported Channel Width"),
			           new QStandardItem("20 or 40 MHz")});
			break;
		}
		...
		appendRow(
		        {new QStandardItem("LSIG TXOP Protection"),
