    ...
    : QStandardItem("VHT Capabilities")
{
	...
	case VhtRxStbc::OneSpatialStream:
		appendRow({new QStandardItem("RX STBC"), new QStandardItem("1 Spatial Stream Supported")});
		break;
	case VhtRxStbc::TwoSpatialStreams:
		appendRow({new QStandardItem("RX STBC"), new QStandardItem("2 Spatial Streams Supported")});
		break;
	case VhtRxStbc::ThreeSpatialStreams:
		appendRow({new QStandardItem("RX STBC"), new QStandardItem("3 Spatial Streams Supported")});
		break;
	case VhtRxStbc::FourSpatialStreams:
		appendRow({new QStandardItem("RX STBC"), new QStandardItem("4 Spatial Streams Supported")});
		break;
	}
	...
	case LinkAdaptation::SupportedUnsolicited:
		appendRow({new QStandardItem("Link Adaptation"), new QStandardItem("Unsolicited Only")});
		break;
	...
	QStandardItem *rx_mcs_map = new QStandardItem("Rx MCS Map");
	std::array<VhtMcs, 8> mcs_rx = vht_cap.mcs_rx();
	for (unsigned long i = 0; i < mcs_rx.size(); ++i) {
		switch (mcs_rx[i]) {
		case VhtMcs::NotSupported:
			rx_mcs_map->appendRow({new QStandardItem(QString("%0 Spatial Stream(s)").arg(i + 1)),
	...
	QStandardItem *tx_mcs_map = new QStandardItem("Tx MCS Map");
	std::array<VhtMcs, 8> mcs_tx = vht_cap.mcs_tx();
	for (unsigned long i = 0; i < mcs_rx.size(); ++i) {
		switch (mcs_tx[i]) {
		case VhtMcs::NotSupported:
			tx_mcs_map->appendRow({new QStandardItem(QString("%0 Spatial Stream(s)").arg(i + 1)),
			                       new QStandardItem("Not Supported")});
			break;
		case VhtMcs::OneThroughSeven:
			tx_mcs_map->appendRow({new QStandardItem(QString("%0 Spatial Stream(s)").arg(i + 1)),
